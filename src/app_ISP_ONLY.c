...
#if 0   //I think this is how it should be enabled. send 0xac 0x53 then wait ~60us then read 2 bytes.
            tx_buffer[0] = 0xac;
            tx_buffer[1] = 0x53;
            spi_buffer_handle = DRV_SPI_BufferAddWrite(SPIHandle, tx_buffer, 2, 0, 0);
            while(DRV_SPI_BUFFER_EVENT_COMPLETE != DRV_SPI_BufferStatus(spi_buffer_handle)); //wait until spi is done         

            BSP_DelayUs(60);

            spi_buffer_handle = DRV_SPI_BufferAddRead(SPIHandle, rx_buffer, 2, 0, 0);
            while(DRV_SPI_BUFFER_EVENT_COMPLETE != DRV_SPI_BufferStatus(spi_buffer_handle)); //wait until spi is done         
...
#if 0   //Lock the device. Can't read device memories via ICE if locked!
